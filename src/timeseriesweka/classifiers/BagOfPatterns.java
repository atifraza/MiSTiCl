...
public class BagOfPatterns extends AbstractClassifierWithTrainingData{
     ...
     * No params given, do parameter search
     */
    public BagOfPatterns(Set<Integer> alphas, Set<Integer> windows) {
        this();
        if (alphas.isEmpty() || windows.isEmpty()) {
            flag = false;
        } else {
            this.alphas = alphas;
     ...
     */
    public int[] parameterSearch(Instances data) throws Exception {
        double bestAcc = 0.0;
...
//        int winInc = 1; //check every size in range
        ...
        if (flag) {
            int winSize;
            for (Integer alphaSize : alphas) {
                for (Integer window : windows) {
                    winSize = (int)((data.numAttributes()-1) / window);
                    for (int wordSize = 2; wordSize <= winSize/2; wordSize*=2) { //lin BoP suggestion
                        BagOfPatterns bopLocal = new BagOfPatterns(wordSize, alphaSize, winSize);
                        double acc = bopLocal.crossValidate(data); //leave-one-out without rebuiding every fold
